...
const buildFallbackReport = (interview) => {
  ...
  return {
    ...
    interviewDuration: interview.duration || `${Math.max(5, Math.ceil((userAnswers.length + aiQuestions.length) * 1.5))}-${Math.max(8, Math.ceil((userAnswers.length + aiQuestions.length) * 2))} minutes`,
...
- Difficulty: ${interview.difficulty}
- Interview Duration: ${interview.duration || '15 mins'}
...
Provide a comprehensive analysis in the following JSON format (respond ONLY with valid JSON, no additional text):
{
  ...
  "interviewDuration": "${interview.duration || '15 mins'}",
